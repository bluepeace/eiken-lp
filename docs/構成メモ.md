...
- **Webアプリ**: app.aiken.life（別リポジトリ eiken-app）
- **iPhone / iPadアプリ**: [App Store（AiKen）](https://apps.apple.com/jp/app/aiken/id6814042355)（2026年9月公開）
- **Androidアプリ**: 近日公開（iOS と同様に Google Play 配信予定）
- **技術**: PHP + Tailwind CDN。共通部品は `include` で組み立て。ビルド不要。

ストア ID・課金・未成年契約などの詳細は **[ストア公開.md](./ストア公開.md)** を参照。
...
│   ├── ストア公開.md          # iOS / Android の公開状況・ストアURL・課金メモ
│   ├── 英検級別基礎情報.md    # 各級の出題形式・レベル・傾向・対策（LP・ブログの基礎情報）
│   ├── 級別キャプチャ画像.md  # 級別LPの画面キャプチャ命名・サイズ・差し替え手順
│   └── blog/                  # コンテンツマーケティング用ブログ記事
│       ├── README.md          # ブログの目的・流れ・トーン
│       ├── research/           # 記事作成前の調査メモ（ファクトベース）
│       │   ├── 01_英検2026年度日程_調査メモ.md
│       │   ├── 02_英検前日持ち物_調査メモ.md
│       │   └── 03_英検級レベル_調査メモ.md
│       └── posts/             # WordPress用記事本文（HTML）
│           ├── 01_英検2026年度_日程スケジュール.html
│           ├── 02_英検_前日確認_持ち物.html
│           └── 03_英検_級_レベル.html
├── includes/
...
|------|-----|------|
| TOP | `/` | index.php |
| AiKenとは | `/about`（`about.php`） | about.php |
| よくあるご質問 | `/faq`（`faq.php`） | faq.php |
| 料金・プラン | `/plan`（`plan.php`） | plan.php |
| サイトマップ | `/sitemap.xml`（`sitemap.php`） | sitemap.php |
| 特定商取引法（noindex） | `/tokushoho`（`tokushoho.php`） | tokushoho.php |
| 利用規約 | `/terms`（`terms.php`） | terms.php |
| プライバシーポリシー | `/privacy`（`privacy.php`） | privacy.php |
...
| SITE_URL, APP_URL | LP / WebアプリのURL。ネイティブは [ストア公開.md](./ストア公開.md) |
| BRAND_COLOR | キーカラー（#50c2cb） |
| MONTHLY_PRICE | 月額プラン料金（税込・円）。現在 **980** |
| FREE_TRIAL_DAYS | 無料体験日数。現在 **5** |
...
- **級別キャプチャ**（単語・R/L/W/S）：`assets/images/grade/{slug}/{key}.webp` / `.jpg` / `.png`（webp → jpg → png）に置くだけで差し替え。詳細は **[級別キャプチャ画像.md](./級別キャプチャ画像.md)** を参照。
