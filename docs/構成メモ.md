...
│   └── sections/
│       ├── hero.php           # TOP：ヒーロー
│       ├── intro.php          # TOP：英検のメリット・AiKenでできること（SEO）
│       ├── problems.php       # TOP：悩み
...
2. `config.php` → `header.php` → 各 section（hero → intro → problems → features → target → grade_links → benefits → howto → faq → cta）→ `footer.php`
